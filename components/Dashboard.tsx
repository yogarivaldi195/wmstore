...
export const Dashboard: React.FC<DashboardProps> = ({ items, transactions, lang, t }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[var(--bg-secondary)]/95 border border-[var(--border-primary)] p-4 rounded-xl shadow-2xl backdrop-blur-xl z-50">
          <p className="text-[var(--text-secondary)] text-xs mb-2 font-mono uppercase tracking-wider">{label}</p>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] tracking-tight font-[Space_Grotesk] text-glow">
        ...
        <div className="flex gap-3 w-full md:w-auto">
           <div className="glass-panel flex-1 md:flex-none px-5 py-3 rounded-2xl flex items-center gap-3 border-indigo-500/20">
              ...
              <div className="text-xs">
                <p className="text-[var(--text-secondary)] font-bold uppercase">Total SKUs</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:perspective-[1000px]">
        ...
        <div className="card-3d glass-panel p-6 rounded-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <p className="text-[var(--text-secondary)] text-xs font-bold uppercase tracking-widest">{t.assetValue}</p>
                {/* Menggunakan formatCurrency (Rp) */}
                <h3 className="text-2xl xl:text-3xl font-bold text-[var(--text-primary)] mt-2 font-[Space_Grotesk] truncate">{formatCurrency(stats.totalValue)}</h3>
        ...
        <div className="card-3d glass-panel p-6 rounded-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <p className="text-[var(--text-secondary)] text-xs font-bold uppercase tracking-widest">Inventory Health</p>
                <h3 className="text-2xl xl:text-3xl font-bold text-[var(--text-primary)] mt-2 font-[Space_Grotesk]">{stats.stockHealth}%</h3>
                <div className="w-full bg-[var(--bg-overlay)] h-1.5 mt-4 rounded-full overflow-hidden">
                ...
                <p className="text-[var(--text-tertiary)] text-xs mt-2">{stats.lowStockCount} items below minimum</p>
            </div>
        ...
        <div className="card-3d glass-panel p-6 rounded-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <p className="text-[var(--text-secondary)] text-xs font-bold uppercase tracking-widest">{t.totalStock}</p>
                {/* Angka dihitung dari banyaknya material_no */}
                <h3 className="text-2xl xl:text-3xl font-bold text-[var(--text-primary)] mt-2 font-[Space_Grotesk]">{stats.uniqueMaterialCount.toLocaleString()}</h3>
                <p className="text-[var(--text-tertiary)] text-xs mt-4">Unique Materials / SKUs</p>
            </div>
        ...
        <div className="card-3d glass-panel p-6 rounded-3xl relative overflow-hidden group bg-rose-900/10 border-rose-500/20">
            ...
            <div className="relative z-10">
                <p className="text-rose-400 dark:text-rose-300 text-xs font-bold uppercase tracking-widest">Est. Restock Cost</p>
                {/* Menggunakan formatCurrency (Rp) */}
                <h3 className="text-2xl xl:text-3xl font-bold text-[var(--text-primary)] mt-2 font-[Space_Grotesk]">{formatCurrency(stats.restockCostEstimates)}</h3>
                <p className="text-rose-400 dark:text-rose-300/70 text-xs mt-4">To restore min levels</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-panel p-6 rounded-3xl relative overflow-hidden border border-[var(--border-secondary)]">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className="text-lg font-bold text-[var(--text-primary)] flex items-center gap-2">
                            <Activity className="w-5 h-5 text-indigo-400" /> {t.flowAnalytics}
                        </h3>
                        <p className="text-xs text-[var(--text-tertiary)]">Volume In/Out (Last 7 Days)</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Stockout Prediction */}
                <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)]">
                    <div className="flex items-center gap-2 mb-4">
                        <Hourglass className="w-5 h-5 text-amber-400" />
                        <h3 className="text-lg font-bold text-[var(--text-primary)]">Risk Forecast</h3>
                    </div>
                    <div className="space-y-3">
                        {riskItems.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl">
                                <div>
                                    <p className="text-sm font-bold text-[var(--text-primary)] truncate max-w-[120px]">{item.name}</p>
                ...
                <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)]">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-lg font-bold text-[var(--text-primary)]">Slow Moving</h3>
                    </div>
                    <div className="space-y-3">
                        {slowMovingItems.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-[var(--bg-overlay)] border border-[var(--border-secondary)] rounded-xl">
                                <div>
                                    <p className="text-sm font-bold text-[var(--text-primary)] truncate max-w-[150px]">{item.name}</p>
                                    <p className="text-[10px] text-[var(--text-tertiary)]">SLOC: {item.sloc}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-bold text-[var(--text-primary)] font-mono">{item.quantity}</p>
                                    <p className="text-[10px] text-[var(--text-tertiary)]">Idle Units</p>
                                </div>
                            </div>
                        ))}
            ...
            <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)] relative">
                <div className="flex items-center gap-2 mb-6">
                    <Wallet className="w-5 h-5 text-emerald-400" />
                    <div>
                        <h3 className="text-lg font-bold text-[var(--text-primary)]">Capital Distribution</h3>
                        <p className="text-xs text-[var(--text-tertiary)]">Asset Value by Category</p>
                    </div>
                </div>
                <div className="h-[200px]">
                     <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={categoryValueData}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border-secondary)" />
                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: 'var(--text-tertiary)', fontSize: 10}} dy={10} />
                            <YAxis axisLine={false} tickLine={false} tickFormatter={(val) => `${(val/1000000).toFixed(0)}M`} tick={{fill: 'var(--text-tertiary)', fontSize: 10}} />
                            <Tooltip
        ...
        <div className="space-y-6">
            ...
            <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)] relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 blur-[60px] rounded-full"></div>
                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-2">{t.composition}</h3>
                <p className="text-xs text-[var(--text-tertiary)] mb-6">Quantity Distribution</p>
                <div className="h-[220px] relative">
                     <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Legend verticalAlign="bottom" height={36} iconType="circle" formatter={(value) => <span className="text-[var(--text-secondary)] text-[10px] font-mono ml-1">{value}</span>} />
                        </PieChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)]">
                 ...
                 <div className="space-y-4">
                         ...
                         <div key={idx} className="flex items-center gap-3">
                             ...
                             <div className="flex-1">
                                 <div className="flex justify-between mb-1">
                                     <span className="text-xs text-[var(--text-primary)] font-bold truncate max-w-[120px]">{item.name}</span>
                                     <span className="text-xs text-yellow-400 font-mono">{item.count} units</span>
                                 </div>
                                 <div className="w-full bg-[var(--bg-overlay)] h-1 rounded-full overflow-hidden">
                     ...
                     {topMovers.length === 0 && (
                       <p className="text-[var(--text-tertiary)] text-xs text-center">No transaction data available.</p>
                     )}
                 </div>
            </div>

            {/* Warehouse Status Summary */}
            <div className="glass-panel p-6 rounded-3xl border border-[var(--border-secondary)] bg-gradient-to-br from-indigo-900/20 to-purple-900/20 dark:from-indigo-900/20 dark:to-purple-900/20 from-indigo-100/20 to-purple-100/20">
                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Warehouse Status</h3>
                <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30">
                        <CheckCircle2 className="w-6 h-6 text-emerald-400" />
                    </div>
                    <div>
                        <p className="text-sm text-[var(--text-primary)] font-bold">Operational</p>
                        ...
                        <p className="text-xs text-[var(--text-secondary)]">Tracking {stats.uniqueMaterialCount} active materials</p>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                    <div className="bg-[var(--bg-overlay)] p-2 rounded-lg text-center">
                         <p className="text-[10px] text-[var(--text-tertiary)] uppercase">Inbound (7d)</p>
                         <p className="text-emerald-400 font-mono font-bold text-sm">
                            {chartData.reduce((acc, d) => acc + d.inbound, 0)}
                         </p>
                    </div>
                    <div className="bg-[var(--bg-overlay)] p-2 rounded-lg text-center">
                         <p className="text-[10px] text-[var(--text-tertiary)] uppercase">Outbound (7d)</p>
