...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden md:perspective-[2000px] bg-[var(--bg-primary)]">

      {/* Dynamic Background for Login */}
      <div className="absolute inset-0 z-0">
        <div className="absolute top-[20%] left-[20%] w-96 h-96 bg-indigo-600/20 dark:bg-indigo-600/20 bg-indigo-400/10 rounded-full blur-[120px] animate-pulse"></div>
        <div className="absolute bottom-[20%] right-[20%] w-96 h-96 bg-purple-600/20 dark:bg-purple-600/20 bg-purple-400/10 rounded-full blur-[120px] animate-pulse delay-1000"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md transform transition-all duration-500 md:hover:rotate-x-2 md:hover:rotate-y-2 md:preserve-3d group px-4">
        ...
        <div className="glass-panel p-1 rounded-[2.5rem] backdrop-blur-xl border border-[var(--border-primary)] shadow-[0_0_80px_rgba(99,102,241,0.3)] relative overflow-hidden">
          ...
          <div className="bg-[var(--bg-secondary)]/80 rounded-[2.3rem] p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="text-center mb-8 relative">
              <div className="relative inline-block mb-4 group-hover:scale-110 transition-transform duration-500">
                ...
                <div className="relative bg-gradient-to-br from-indigo-600 to-violet-600 p-4 rounded-2xl shadow-lg border border-white/20">
                  <Hexagon className="w-10 h-10 text-[var(--text-primary)] fill-white/20" />
                </div>
              ...
              <h1 className="text-3xl font-bold text-[var(--text-primary)] tracking-tight font-[Space_Grotesk] text-glow">eSTORE</h1>
              <div className="flex items-center justify-center gap-2 mt-2 opacity-60">
                <span className="w-1 h-1 bg-indigo-400 rounded-full"></span>
                <p className="text-xs font-mono tracking-[0.2em] text-indigo-400 dark:text-indigo-300">SECURE ACCESS GATEWAY</p>
                <span className="w-1 h-1 bg-indigo-400 rounded-full"></span>
              </div>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-5 relative z-10">
              ...
              {isSignUp && (
                <div className="group relative animate-in slide-in-from-bottom-4 fade-in duration-300">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-tertiary)] group-focus-within:text-indigo-400 transition-colors">
                  ...
                  <input
                    ...
                    className="w-full bg-[var(--bg-overlay)] border border-[var(--border-secondary)] rounded-xl py-4 pl-12 pr-4 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 outline-none transition-all font-mono text-sm"
                    required={isSignUp}
                  />
                </div>
              )}
            ...
            {/* Footer / Toggle Mode */}
            <div className="mt-6 pt-6 border-t border-[var(--border-secondary)] text-center">
               <button
                 onClick={() => { setIsSignUp(!isSignUp); setError(null); setName(''); }}
                 className="text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-mono transition-colors flex items-center justify-center gap-2 w-full"
               >
                 {isSignUp ? (
            ...
            <div className="mt-4 flex justify-between items-center text-[10px] text-[var(--text-tertiary)] font-mono">
