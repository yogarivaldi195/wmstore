...
export const Inventory: React.FC<InventoryProps> = ({ items, setItems, userRole, currentUser, lang, t }) => {
  ...
  const renderDetailContent = (item: InventoryItem) => (
    <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 border-t border-[var(--border-secondary)] bg-indigo-950/5 inner-shadow text-sm">
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 p-3 md:p-4 rounded-xl bg-[var(--bg-overlay)] border border-[var(--border-secondary)]">
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">PR Status</p>
                    <p className="text-[var(--text-primary)] font-mono text-xs md:text-sm">{item.prStatus || 'N/A'}</p>
                </div>
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">PR Number</p>
                    <p className="text-[var(--text-primary)] font-mono text-xs md:text-sm">{item.prNumber || '-'}</p>
                </div>
                <div className="col-span-2">
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">WBS Element</p>
                    <p className="text-[var(--text-primary)] font-mono text-xs md:text-sm">{item.wbs || 'Not Assigned'}</p>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 p-3 md:p-4 rounded-xl bg-[var(--bg-overlay)] border border-[var(--border-secondary)]">
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">Min Stock</p>
                    <p className="text-rose-400 font-mono font-bold text-xs md:text-sm">{item.minStock}</p>
                </div>
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">Max Stock</p>
                    <p className="text-emerald-400 font-mono font-bold text-xs md:text-sm">{item.maxStock || '-'}</p>
                </div>
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">Unit Cost (Avg)</p>
                    <p className="text-[var(--text-primary)] font-mono text-xs md:text-sm">{formatCurrency(calculateUnitPrice(item.price, item.quantity))}</p>
                </div>
                <div>
                    <p className="text-[var(--text-tertiary)] text-[10px] uppercase">Class</p>
                    <p className="text-[var(--text-primary)] font-mono text-xs md:text-sm">{item.category}</p>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-3">
            ...
            <div className="p-3 md:p-4 rounded-xl bg-[var(--bg-overlay)] border border-[var(--border-secondary)] h-[140px] overflow-y-auto custom-scrollbar">
                    ...
                    {item.history.slice(0, 5).map((log, i) => (
                        <li key={i} className="text-xs border-l-2 border-indigo-500/30 pl-3">
                            <div className="flex justify-between">
                                ...
                                <span className="text-[var(--text-tertiary)]">{log.date.split(',')[0]}</span>
                            </div>
                            <p className="text-[var(--text-secondary)] mt-0.5 truncate">{log.details}</p>
                            <p className="text-[var(--text-tertiary)] text-[10px] mt-0.5">by {log.user}</p>
                        </li>
                    ))}
                    {item.history.length === 0 && <li className="text-[var(--text-tertiary)] italic">No history recorded</li>}
                </ul>
            </div>
        </div>
    </div>
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 min-h-[80vh] relative" onClick={() => setIsColumnMenuOpen(false)}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 md:perspective-[1000px]">
        <div className="card-3d origin-left">
          <h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] tracking-tight font-[Space_Grotesk] text-glow">eSTORE <span className="text-indigo-500">Stock</span></h2>
          <p className="text-[var(--text-secondary)] mt-2 font-mono text-sm">{t.stockMaster}</p>
        </div>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
             {selectedIds.size > 0 && (
               <button 
                 ...
                 className="bg-rose-600 hover:bg-rose-500 text-[var(--text-primary)] px-4 py-4 rounded-2xl font-bold flex items-center gap-2 shadow-lg"
               >
             ...
             <button 
               ...
               className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-[var(--text-primary)] px-8 py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all shadow-[0_0_30px_rgba(99,102,241,0.4)]"
             >
      ...
      {criticalItems.length > 0 && (
        <div className="animate-in slide-in-from-top-4 duration-500 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-[0_0_30px_rgba(245,158,11,0.1)]">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="text-lg font-bold text-[var(--text-primary)]">{t.reorderAlert}</h3>
            ...
            {canEdit && (
                <button 
                    ...
                    className="w-full md:w-auto px-6 py-3 bg-amber-600 hover:bg-amber-500 text-[var(--text-primary)] font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all"
                >
      ...
      <div className="glass-panel p-3 rounded-2xl flex flex-col md:flex-row gap-4 items-center relative z-20 shadow-2xl">
        <div className="relative flex-1 w-full group">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 md:py-4 bg-[var(--bg-overlay)] border border-[var(--border-secondary)] rounded-xl text-[var(--text-primary)] focus:ring-0 placeholder:text-[var(--text-tertiary)] font-mono text-sm transition-all focus:bg-[var(--bg-overlay)]"
            value={searchTerm}
        ...
        <div className="flex gap-2 w-full md:w-auto">
            ...
            <div className="relative hidden md:block">
                <button 
                    ...
                    className={`flex items-center gap-2 px-4 py-3 md:py-4 rounded-xl transition-all border ${isColumnMenuOpen ? 'bg-indigo-600/20 text-indigo-400 border-indigo-500/30' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 border-transparent'}`}
                >
                ...
                {isColumnMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 w-56 bg-[#0a0a0a] border border-[var(--border-secondary)] rounded-xl shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
                        <div className="text-xs font-bold text-[var(--text-tertiary)] uppercase px-3 py-2">Toggle Columns</div>
                        {Object.entries(visibleColumns).map(([key, isVisible]) => (
                            ...
                            >
                                <span className="text-[var(--text-primary)] capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                {isVisible ? <Eye className="w-4 h-4 text-indigo-400" /> : <EyeOff className="w-4 h-4 text-[var(--text-tertiary)]" />}
                            </button>
                        ))}
                    </div>
                )}
            ...
            <button 
                ...
                className={`flex items-center gap-2 px-6 py-3 md:py-4 rounded-xl transition-all justify-center border flex-1 md:flex-none ${showLowStockOnly ? 'bg-rose-500/20 text-rose-400 border-rose-500/30' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 border-transparent'}`}
            >
      ...
      <div className="md:hidden space-y-4">
        {currentItems.map((item) => {
            ...
            return (
                <div key={item.id} className={`glass-panel rounded-2xl overflow-hidden border transition-all ${isExpanded ? 'border-indigo-500/50 shadow-lg' : 'border-[var(--border-secondary)]'}`}>
                    {/* Main Card Content */}
                    <div className="p-4" onClick={() => toggleRowExpansion(item.id)}>
                        <div className="flex items-start justify-between mb-3">
                            <div className="flex items-center gap-3">
                                <button onClick={(e) => { e.stopPropagation(); toggleSelection(item.id); }} className="p-1 -ml-2">
                                    {isSelected ? <CheckSquare className="w-5 h-5 text-indigo-500" /> : <Square className="w-5 h-5 text-[var(--text-tertiary)]" />}
                                </button>
                                <div>
                                    <h3 className="text-[var(--text-primary)] font-bold text-lg leading-tight">{item.name}</h3>
                                    <span className="text-xs text-[var(--text-tertiary)] font-mono">{item.materialNo}</span>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                             <div className="bg-white/5 rounded-lg p-2">
                                 <p className="text-[10px] text-[var(--text-tertiary)] uppercase">Quantity</p>
                                 <p className={`text-lg font-mono font-bold ${isLowStock ? 'text-rose-400' : 'text-emerald-400'}`}>{item.quantity} <span className="text-xs text-[var(--text-tertiary)]">{item.uom}</span></p>
                             </div>
                             <div className="bg-white/5 rounded-lg p-2">
                                 <p className="text-[10px] text-[var(--text-tertiary)] uppercase">Total Valuation</p>
                                 <div className="flex flex-col">
                                    <p className="text-[var(--text-primary)] font-mono font-bold text-sm">{formatCurrency(item.price)}</p>
        ...
        {currentItems.length === 0 && (
            <div className="text-center py-12 text-[var(--text-tertiary)]">No items found.</div>
        )}
      </div>

      {/* =================== DESKTOP VIEW (TABLE) =================== */}
      <div className="hidden md:block glass-panel rounded-3xl overflow-hidden border border-[var(--border-secondary)] shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm min-w-[1000px] border-collapse">
            <thead className="bg-[var(--bg-overlay)] text-xs uppercase tracking-widest text-indigo-300 font-bold font-[Space_Grotesk]">
              <tr>
                <th className="px-4 py-6 w-12 text-center">
                    <button onClick={toggleSelectAll} className="hover:text-[var(--text-primary)] transition-colors">
                    ...
                    >
                      ...
                      {visibleColumns.materialInfo && (
                          <td className="px-4 py-5">
                              <p className="font-bold text-[var(--text-primary)] text-lg group-hover:text-indigo-300 transition-colors">{item.name}</p>
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs bg-white/5 px-2 py-0.5 rounded text-[var(--text-secondary)] border border-[var(--border-secondary)]">{item.category}</span>
                      ...
                      {visibleColumns.identification && (
                          <td className="px-6 py-5">
                              <div className="flex flex-col">
                                  <span className="text-[var(--text-primary)] font-mono text-xs">MAT: {item.materialNo}</span>
                                  {item.wbs && <span className="text-[var(--text-tertiary)] font-mono text-[10px]">WBS: {item.wbs}</span>}
                              </div>
                          </td>
                      )}
                      ...
                      {visibleColumns.location && (
                          <td className="px-6 py-5">
                            <div className="text-xs">
                                <div className="text-[var(--text-primary)] font-bold">{item.sloc}</div>
                                <div className="text-[var(--text-tertiary)] font-mono">{item.rackNo || 'No Rack'}</div>
                            </div>
                          </td>
                      )}
                      ...
                      {visibleColumns.status && (
                          <td className={`px-6 py-5 ${isLowStock ? 'bg-rose-900/10' : ''}`}>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-[var(--text-tertiary)]">{item.uom}</span>
                            </div>
                          </td>
                      )}

                      {visibleColumns.valuation && (
                          <td className="px-6 py-5 text-[var(--text-primary)] font-mono">
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex justify-end gap-2 items-center">
                            <ChevronDown className={`w-4 h-4 text-[var(--text-tertiary)] transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                            {canEdit ? (
                            <>
                                <button onClick={(e) => openEditModal(e, item)} className="p-2 hover:bg-indigo-500/10 text-[var(--text-tertiary)] hover:text-indigo-400 rounded-lg"><Pencil className="w-4 h-4" /></button>
                            </>
                            ) : <Lock className="w-4 h-4 text-[var(--text-tertiary)] ml-auto" />}
                        </div>
                      </td>
                    </tr>
      ...
      {/* PAGINATION CONTROLS */}
      <div className="flex flex-col md:flex-row justify-between items-center p-4 border-t border-[var(--border-secondary)] bg-[var(--bg-overlay)] gap-4 rounded-b-3xl glass-panel mt-0">
           <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
              <span>Rows per page:</span>
              <select 
                 value={itemsPerPage}
                 onChange={(e) => setItemsPerPage(Number(e.target.value))}
                 className="bg-white/5 border border-[var(--border-secondary)] rounded-lg px-2 py-1 text-[var(--text-primary)] outline-none focus:border-indigo-500"
              >
           ...
           <div className="flex items-center gap-2">
              <button 
                ...
                className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-[var(--text-primary)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
                    ...
                    if (pageNum > 0 && pageNum <= totalPages) {
                        return (
                            <button
                                ...
                                className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${currentPage === pageNum ? 'bg-indigo-600 text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-white/5'}`}
                            >
                                {pageNum}
                            </button>
                        )
