    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 min-h-[80vh]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
            ...
            <p className="text-slate-400 text-sm font-mono">Logistics Flow Control</p>
        </div>
        
        <div className="flex gap-4 w-full md:w-auto">
           <button onClick={() => setIsOutboundModalOpen(true)} className="flex-1 md:flex-none px-6 py-3 bg-rose-600 hover:bg-rose-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all hover:scale-105">
               <Send className="w-4 h-4" /> New Outbound
           </button>
           <div className="bg-white/5 p-1 rounded-xl flex">
               <button onClick={() => setActiveTab('OUT')} className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'OUT' ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>Outbound</button>
               <button onClick={() => setActiveTab('IN')} className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'IN' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>Inbound</button>
           </div>
        </div>
      </div>
      ...
      {activeTab === 'OUT' && (
          <div className="space-y-4">
              ...
              {currentData.map((group: any) => {
                  ...
                  return (
                    <div key={g.issueNumber} className={`glass-panel rounded-2xl overflow-hidden border transition-all duration-300 ${isExpanded ? 'border-rose-500/30 shadow-[0_0_30px_rgba(225,29,72,0.1)]' : 'border-white/5'}`}>
                        ...
                        >
                            ...
                            <div className="md:hidden">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
                                    <div className="bg-black/20 p-2 rounded">
                                        <span className="block text-[10px] text-slate-500 uppercase">Receiver</span>
                                        {g.receiver}
                                    </div>
                                    <div className="bg-black/20 p-2 rounded">
                                        <span className="block text-[10px] text-slate-500 uppercase">Project</span>
                                        {g.wbs}
                                    </div>
                                </div>
                            ...
                            <div className="hidden md:grid grid-cols-12 gap-4 items-center">
                                ...
                                <div className="col-span-2">
                                    <span className="px-2 py-1 rounded bg-white/5 text-xs font-mono text-slate-400 border border-white/10">{g.wbs}</span>
                                </div>
                                <div className="col-span-2 text-center">
                                    <span className="text-white font-bold">{g.itemCount}</span> <span className="text-slate-500 text-xs">items</span>
                                </div>
                                <div className="col-span-1 text-right">
                                    <button className={`p-2 rounded-lg transition-all ${isExpanded ? 'bg-rose-500 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                                        ...
                                        <tbody className="divide-y divide-white/5">
                                            {g.items.map((item, idx) => (
                                                <tr key={idx} className="hover:bg-white/5">
                                                    ...
                                                    <td className="px-4 py-3 text-xs font-mono text-slate-400">{item.glAccount}</td>
                ...
                <tbody className="divide-y divide-white/5">
                    {currentData.map((t: any) => (
                        <tr key={t.id} className="hover:bg-white/5">
                            <td className="px-6 py-4 font-mono text-xs text-slate-400">{t.date}</td>
                            <td className="px-6 py-4">
                                <div className="font-bold text-white">{t.itemName}</div>
                            ...
                            <td className="px-6 py-4 text-xs text-slate-500">{t.remark}</td>
                        </tr>
                    ))}
                    {currentData.length === 0 && (
                        <tr><td colSpan={4} className="text-center py-8 text-slate-500">No inbound data.</td></tr>
                    )}
                </tbody>
             </table>
          </div>
      ...
      <div className="flex justify-between items-center bg-black/20 p-4 rounded-2xl border border-white/5">
         ...
         <div className="flex gap-2">
            <button 
                ...
                className="p-2 bg-white/5 hover:bg-white/10 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed text-white transition-colors"
            >
                ...
                <form onSubmit={handleCreateOutbound} className="space-y-5">
                    ...
                    <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className="flex-1">
                                <select required value={selectedMaterialId} onChange={(e) => setSelectedMaterialId(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500 appearance-none">
                                    <option value="">Select Material...</option>
                                    {items?.map(i => <option key={i.id} value={i.id}>{i.name} ({i.materialNo}) | Qty: {i.quantity}</option>)}
                                </select>
                            </div>
                            <div className="w-full md:w-32">
                                <input type="number" min="1" required value={outQty} onChange={e => setOutQty(parseInt(e.target.value))} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white text-center font-mono font-bold focus:border-rose-500" placeholder="Qty" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" required value={formData.issueNumber} onChange={e => setFormData({...formData, issueNumber: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500 font-mono" placeholder="ISS-2024-001" />
                        </div>
                        <div>
                            <label className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase mb-2">
                                <Hash className="w-3 h-3" /> WBS Element
                            </label>
                            <input type="text" value={formData.wbs} onChange={e => setFormData({...formData, wbs: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500 font-mono" placeholder="P-001-X" />
                        </div>
                        <div>
                            <label className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase mb-2">
                                <CreditCard className="w-3 h-3" /> GL Account
                            </label>
                            <input type="text" value={formData.glAccount} onChange={e => setFormData({...formData, glAccount: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500" placeholder="Cost Center" />
                        </div>
                        <div>
                            <label className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase mb-2">
                                <User className="w-3 h-3" /> Receiver / User
                            </label>
                            <input type="text" required value={formData.goodReceipt} onChange={e => setFormData({...formData, goodReceipt: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500" placeholder="Name / Dept" />
                        </div>
                    </div>
                    
                    <div>
                        <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Remarks</label>
                        <textarea value={formData.remarks} onChange={e => setFormData({...formData, remarks: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-rose-500 h-20 resize-none" placeholder="Additional notes..."></textarea>
                    </div>
                    
                    <div className="pt-4 flex justify-end gap-3 sticky bottom-0 bg-[#0a0a0a] pb-2">
                         <button type="button" onClick={() => setIsOutboundModalOpen(false)} className="px-6 py-3 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 font-bold">Cancel</button>
